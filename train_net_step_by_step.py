...
name = 'model=step_by_step%s-kernel_mvmt_size=%d-mvmt=%d-max_step=%d-sd_weight=%.3f-lr=%.4f' % (opt.model, opt.mvmt_kernel_size, opt.mvmt_dim, opt.max_step, opt.sd_weight, opt.lr)
...
# --------- training funtions ------------------------------------
def train(x, train_discriminator):
    netEM.zero_grad()
    netD.zero_grad()
...
def eval_scene_discriminator(x):
    target = torch.cuda.FloatTensor(opt.batch_size, 1)
    #target = torch.FloatTensor(opt.batch_size, 1)
...
for epoch in range(80, 120):
    ...
    for i in range(opt.epoch_size):
        ...
        netC.eval()
        # train scene discriminator
        sd_loss, sd_acc = eval_scene_discriminator(x)
        epoch_sd_loss += sd_loss
        epoch_sd_acc += sd_acc
        netC.train()

        # train main model
        rec_loss = train(x, True)
#        print(rec_loss)
        epoch_rec_loss += rec_loss
